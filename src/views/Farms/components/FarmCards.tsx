...
const FarmCards: React.FC = () => {
  ...
  const dbaIndex = farms.findIndex(
    ({ tokenSymbol }) => tokenSymbol === 'DBA',
  )

  const dbaPrice =
    dbaIndex >= 0 && stakedValue[dbaIndex]
      ? stakedValue[dbaIndex].tokenPriceInWeth
      : new BigNumber(0)

  const BLOCKS_PER_YEAR = new BigNumber(2372500)
  const DBA_PER_BLOCK = new BigNumber(60)
...
const FarmCard: React.FC<FarmCardProps> = ({ farm }) => {
  ...
  }, [dba, lpTokenAddress, account, setHarvestable])

  const poolActive = true // startTime * 1000 - Date.now() <= 0

  return (
    <StyledCardWrapper>
      {farm.tokenSymbol === 'DBA' && <StyledCardAccent />}
      <Card>
        <CardContent>
          <StyledContent>
